...
- Radius standardization is **code hygiene only** — square-mode (`globals.css`) forces all container radii to `0`, so normalizing `rounded-*` classes changes no pixels. Corners are square by brand intent and that is permanent.
...
- `--maxw` (1180) is retired; document pages cap at `--wide` (1320), matching what the code already did. DESIGN.md §5 corrected to match.
