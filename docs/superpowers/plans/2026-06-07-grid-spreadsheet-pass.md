# Grid Spreadsheet Pass Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add seven Excel/Sheets/Airtable-inspired interactions to `DataGrid` — fill handle (vertical only), ⌘+Arrow data-edge jump, status-bar live aggregates, click row#/header to select, right-click context menu, conditional formatting, and field description tooltips.
...
| `app/src/components/datagrid/DataGrid.tsx` | Wire everything; row#/header click select; render handle/status-bar/menu; apply rule styles |
...
## Task 1 — Drizzle migration: add `dimension_field.description`
...
## Task 3 — Click row#/column-header to select
...
Expected: FAIL — clicks on row# and header label don't set range.
...
git commit -m "feat(grid): click row#/column-header to select whole row/col
...
## Task 4 — Fill handle (vertical only)
...
## Task 5 — Status-bar live aggregates
...
cd app && bun run dev
# create rule via header menu → set "status equals conflict" → "rose" stripe → verify row glows
```
...
## Task 8 — Field description tooltips
...
```bash
cd app && bun run dev
# in a table: column header ⋯ menu → Edit description → type some text → save
# hover the header → small `i` appears → hover it → tooltip shows the text
# refresh the page → description persisted
```
...
```bash
git status
# If clean: nothing to commit. If fixes: commit them with descriptive messages.
```
