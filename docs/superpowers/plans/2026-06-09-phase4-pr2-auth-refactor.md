# Phase 4 PR 2 — Auth Refactor + API Tokens Implementation Plan
...
```
server/
  package.json                                  # MODIFIED — adds openid-client
server/drizzle/
  schema.ts                                     # MODIFIED — users.password_hash + auth_provider; new api_tokens table
  migrations/000N_phase4_auth.sql               # GENERATED via bun run db:generate
server/src/
  env.ts                                        # MODIFIED — adds OIDC_*, derives authMode
  auth.ts                                       # REWRITTEN — thin coordinator + getSessionUser + handleAuthConfig + handleLogout + handleMe + handleDevLogin
  auth-password.ts                              # NEW — handleSignup, handleLogin, handleChangePassword
  auth-oidc.ts                                  # NEW — discovery + handleOidcStart + handleOidcCallback (uses openid-client v6)
  auth-api-tokens.ts                            # NEW — token utils + handleListTokens/handleCreateToken/handleRevokeToken + getApiTokenUser middleware
  server.ts                                     # MODIFIED — route dispatch + bearer-token fallback in session gate
server/scripts/
  reset-password.ts                             # NEW — Bun CLI script for admin password reset
server/test/
  auth-password.test.ts                         # NEW
  auth-oidc.test.ts                             # NEW (mocks openid-client config)
  auth-api-tokens.test.ts                       # NEW
  auth-config.test.ts                           # NEW (small — tests /api/auth/config response shape per mode)
  reset-password.test.ts                        # NEW (small — verifies the CLI updates the hash)
app/src/
  store.ts                                      # MODIFIED — adds AuthConfig + useAuthConfig + API token actions; deprecates allowedDomain from WorkspaceInfo
  main.tsx                                      # MODIFIED — adds /signup route
  routes/Login.tsx                              # REWRITTEN — fetches /api/auth/config; renders password form OR SSO button
  routes/Signup.tsx                             # NEW — email + password + name form
  routes/Settings.tsx                           # MODIFIED — adds "API tokens" Section + read allowedDomain from useAuthConfig instead of useWorkspaceInfo
app/test/
  login-mode-aware.test.tsx                     # NEW — Login renders right UI per config mode
  signup.test.tsx                               # NEW — Signup form validation + submit
  api-tokens-settings.test.tsx                  # NEW — list, create-once-show, revoke flows
```
...
## Task 2: Drizzle migration — `password_hash`, `auth_provider`, `api_tokens` table
...
### Step 5 — Add post-migration data backfill
...
## Task 3: env.ts — `OIDC_*` env vars + `authMode` derivation
...
Note `allowedDomain` falls back to `OIDC_ALLOWED_DOMAIN` if `ALLOWED_DOMAIN` isn't set — this lets BC migrate by just setting the OIDC vars; their existing `ALLOWED_DOMAIN=bettercollective.com` would still work if set.
...
# === Authentication ===
#
# Two modes (one-or-the-other per deployment):
#
# 1) PASSWORD MODE (default when OIDC_ISSUER_URL is unset):
#    Local email + password. First user to sign up becomes the admin and
#    is added to the allowlist. Subsequent users must be invited by an
#    admin in Settings → Team.
#
# 2) OIDC MODE (when OIDC_ISSUER_URL is set):
#    Single-sign-on via any OpenID Connect provider (Google Workspace,
#    Okta, Authentik, Keycloak, etc.). The "Sign in with SSO" button
#    appears on the login page.
#
# Set the OIDC_* env vars below to enable OIDC mode. Example for Google:
#   OIDC_ISSUER_URL=https://accounts.google.com
#   OIDC_CLIENT_ID=...your.google.client.id...
#   OIDC_CLIENT_SECRET=...your.google.client.secret...
#   OIDC_ALLOWED_DOMAIN=example.com    (optional; restricts signups)
#   OIDC_LABEL=Google                  (optional; shown on the SSO button)
...
# Legacy: ALLOWED_DOMAIN. Restricts signups in BOTH password and OIDC modes.
# Falls back to OIDC_ALLOWED_DOMAIN if unset.
ALLOWED_DOMAIN=

# Deprecated: GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET.
# These were used by the Google-OAuth-only path that's been replaced by
# generic OIDC. If you previously used Google OAuth, migrate by setting:
#   OIDC_ISSUER_URL=https://accounts.google.com
#   OIDC_CLIENT_ID=<your existing GOOGLE_CLIENT_ID>
#   OIDC_CLIENT_SECRET=<your existing GOOGLE_CLIENT_SECRET>
# The GOOGLE_* vars are no longer read.
```
...
## Task 4: `GET /api/auth/config` endpoint + tests
...
### Step 3 — Rewrite `handleAuthConfig` in auth.ts
...
## Task 5: `auth-password.ts` — signup, login, change-password endpoints + tests
...
### Step 1 — Create auth-password.ts
...
### Step 2 — Add `issueSession` helper to auth.ts
...
### Step 3 — Wire routes temporarily in server.ts
...
### Step 6 — Typecheck + lint + format + full server tests
...
## Task 6: `auth-oidc.ts` — `openid-client` wrapper + start + callback + tests
...
### Step 2 — Wire new routes in server.ts
...
### Step 5 — Typecheck + lint + format + full server tests
...
## Task 7: `auth-api-tokens.ts` — endpoints + middleware + tests
...
### Step 1 — Create auth-api-tokens.ts
...
### Step 2 — Wire routes in server.ts
...
## Task 8: Bearer-token fallback in server.ts session gate

**Files:**
- Modify: `server/src/server.ts`
- Create: `server/test/auth-bearer-integration.test.ts`

### Step 1 — Update session gate
...
### Step 3 — Run, verify pass (or skip if server not running)
...
### Step 4 — Typecheck + lint + format + full server tests
...
## Task 9: `auth.ts` rewrite — thin coordinator (OPUS)
...
### Step 1 — Inventory what stays / goes / moves
...
### Step 3 — Clean up server.ts route block
...
### Step 6 — Grep gate — Google-specific handlers gone
...
## Task 10: Frontend — `useAuthConfig` hook + deprecate `WorkspaceInfo.allowedDomain`
...
### Step 1 — Add `useAuthConfig` hook in store.ts
...
### Step 2 — Remove `allowedDomain` from `WorkspaceInfo`
...
### Step 3 — Remove `allowedDomain` from server's `/api/workspace/info`
...
### Step 5 — Update Settings.tsx to use `useAuthConfig`
...
### Step 6 — Update any other consumers of `wsInfo.allowedDomain`
...
## Task 11: `Login.tsx` rewrite — mode-aware
...
## Task 12: `Signup.tsx` + route + tests
...
### Step 4 — Register route in main.tsx
...
## Task 13: Settings — "API tokens" Section + tests
...
### Step 1 — Add API token actions to store.ts
...
### Step 4 — Add `ApiTokensSection` to Settings.tsx
...
## Task 14: `scripts/reset-password.ts` — admin CLI
...
### Step 1 — Grep: Google-specific code is gone
...
### Step 2 — Grep: `WorkspaceInfo.allowedDomain` is gone
...
### Step 7 — Drizzle migration smoke
...
### Step 8 — Password-mode boot smoke
...
### Step 9 — OIDC-mode boot smoke (no live provider)
...
### Step 12 — BC migration recommendation note
