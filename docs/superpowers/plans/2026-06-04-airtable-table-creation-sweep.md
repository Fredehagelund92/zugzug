# Airtable-style table creation sweep — Implementation Plan
...
# PHASE A · Foundation (schema, palette, shared types)

### Task 1: Add `description` and `color` to `app.dimension`
...
### Task 2: Curated palette CSS tokens
...
### Task 4: Client types — `OptionDef`, `PaletteName`, dimension extensions
...
# PHASE B · Server changes

### Task 5: `listFields` reads both shapes; new internal helper
...
### Task 6: `addField`, `addColumnOption`, `changeColumnType` accept `OptionDef[]`
...
### Task 7: `silent` flag on `addDimension`, `addSource`, `deriveCanonical`
...
### Task 8: `getDimension` returns `description` and `color`
...
### Task 9: `tables.ts` orchestrator + `POST /api/tables` route
...
### Task 10: `verify-tables.ts` end-to-end script
...
# PHASE C · Client primitives

### Task 11: `Chip` accepts color
...
### Task 12: `SelectCell` renders color; editor picks color on `+ option`
...
### Task 13: `ColumnHeaderMenu` shows option colors + lets you pick on add
...
### Task 14: Shared `OptionBuilder` component
...
# PHASE D · Client store

### Task 15: `createTable()` in store + extend cached dimension shape
...
# PHASE E · CreateTableModal

### Task 16: `CreateTableModal` shell — identity, palette, mode segment
...
### Task 17: `CreateTableModal` — the swappable region (blank columns, source, from IDs)
...
# PHASE F · TablePicker rename + visuals

### Task 18: Rename `DimensionPicker` → `TablePicker` + update imports
...
### Task 19: `TablePicker` visual updates (tint, description, "New table" → modal)
  ...
  if (color) {
    ...
    return (
      <div
        ...
        style={{ background: active ? tint.bg : tint.wash, color: active ? "#FFFFFF" : tint.fg }}
      >
...
# PHASE G · In-grid AddColumn upgrade

### Task 20: In-grid `+ field` widget supports `select` + uses `OptionBuilder`
...
# PHASE H · Naming sweep & capitalization

### Task 21: Rename `NoDimensionsYet` → `NoTablesYet`, update copy
...
### Task 22: Naming + capitalization sweep across routes
...
### Task 23: Full verification + final smoke
