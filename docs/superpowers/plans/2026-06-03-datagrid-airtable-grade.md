***REMOVED*** DataGrid: Airtable-grade inline CRUD — Implementation Plan
...
***REMOVED******REMOVED*** Verification approach
...
***REMOVED*** Phase 1 — DataGrid primitive (foundation)
...
***REMOVED******REMOVED*** Task 1: Bucket hash + Chip primitive
...
***REMOVED******REMOVED*** Task 2: Column / cursor types
...
***REMOVED******REMOVED*** Task 3: `useGridCursor` hook
...
***REMOVED******REMOVED*** Task 4: UndoStack provider
...
***REMOVED******REMOVED*** Task 5: Built-in cells (Text/Number/Boolean/Date)
...
***REMOVED******REMOVED*** Task 6: `DataGrid.tsx` — layout + body (no header menu yet)
...
***REMOVED******REMOVED*** Phase 1 verification
...
***REMOVED*** Phase 2 — Single-select column type, persistence, and MasterTables refactor
...
***REMOVED******REMOVED*** Task 7: Schema migration — `dimension_field.options`
...
***REMOVED******REMOVED*** Task 8: `repo.ts` — `FieldDef.options`, `listFields` projection, `addColumnOption`
...
***REMOVED******REMOVED*** Task 9: API routes — option-add + the addField `options` parameter
...
***REMOVED******REMOVED*** Task 10: `SelectCell.tsx` — chip rendering + picker
...
***REMOVED******REMOVED*** Task 11: Client types + store mutation for option-add
...
***REMOVED******REMOVED*** Task 12: Refactor `MasterTables` body to use `<DataGrid>`
...
***REMOVED******REMOVED*** Task 13: `verify-datagrid.ts` — regression harness (Phase 2 portion)
...
***REMOVED*** Phase 3 — Column header menu, resize + reorder, persisted grid layout
...
***REMOVED******REMOVED*** Task 14: Schema — `user_grid_layout` table
...
***REMOVED******REMOVED*** Task 15: `repo.ts` — getGridLayout / setGridLayout + column rename / change-type / delete
...
***REMOVED******REMOVED*** Task 16: API routes — grid-layout + column mutations
...
***REMOVED******REMOVED*** Task 17: Client store — column-mutation + grid-layout
...
***REMOVED******REMOVED*** Task 18: `ColumnHeaderMenu` component
...
***REMOVED******REMOVED*** Task 19: Wire header menu + sort into `DataGrid.tsx`
...
***REMOVED******REMOVED*** Task 20: Column resize handler (right-edge grip)
...
***REMOVED******REMOVED*** Task 21: Column reorder (200ms hold + drag)
...
***REMOVED******REMOVED*** Task 22: Persist grid layout from MasterTables (load + write-through)
...
```bash
cd app && bun run typecheck
cd app && bun run dev   ***REMOVED*** in one terminal
```
...
***REMOVED******REMOVED*** Task 23: `verify-datagrid.ts` — Phase 3 portion
...
***REMOVED******REMOVED*** Phase 3 verification
...
***REMOVED*** Phase 4 — Wire Mapping to shared primitives
...
***REMOVED******REMOVED*** Task 24: Mapping — focus cursor + grid-style keyboard wrapper
...
***REMOVED******REMOVED*** Task 25: Mapping — undo for accept / skip / reset / pick
...
***REMOVED******REMOVED*** Task 26: Mapping — status chip via shared `<Chip>` primitive
...
***REMOVED******REMOVED*** Task 27: Mapping — remove per-row icon buttons (selection-bar only)
...
***REMOVED*** Phase 5 — Shortcuts overlay + inline shortcut text + focused-row hint
...
***REMOVED******REMOVED*** Task 28: `ShortcutsOverlay` component
...
***REMOVED******REMOVED*** Task 29: Global `?` mount + inline shortcuts on Publish / Undo
...
***REMOVED******REMOVED*** Task 30: Mapping focused-row hint strip
...
***REMOVED******REMOVED*** Phase 5 verification
...
***REMOVED*** Final cross-cutting verification
...
***REMOVED*** Notes for the implementer
