***REMOVED*** Phase 3 — Canonical-Store Modes Implementation Plan
...
server/src/warehouse/
  parquet-exporter.ts          ***REMOVED*** NEW — lazy DuckDB + map-to-parquet via Appender
server/src/
  repo-drafts.ts               ***REMOVED*** MODIFIED — commit() branches on isWritable(adapter)
  server.ts                    ***REMOVED*** MODIFIED — adds /api/dimensions/:id/snapshot.parquet + /api/workspace/info
server/test/
  parquet-exporter.test.ts     ***REMOVED*** NEW
  workspace-info.test.ts       ***REMOVED*** NEW (or merge into an existing endpoint-level test if one fits)
  commit-warehouse-branch.test.ts  ***REMOVED*** NEW — exercises commit() in writable mode with mocked adapter
app/src/
  store.ts                     ***REMOVED*** MODIFIED — adds useWorkspaceInfo + warehouse sync status derivation
  routes/Dashboard.tsx         ***REMOVED*** MODIFIED — adds canonical-destination KPI + needs-resync per-dim badge
  routes/Triage.tsx            ***REMOVED*** MODIFIED — commit-affordance copy adapts to mode
  routes/MasterTables.tsx      ***REMOVED*** MODIFIED — adds "Download snapshot" link in dim header
app/test/
  workspace-info.test.ts       ***REMOVED*** NEW — tests for useWorkspaceInfo
  dashboard-canonical-badge.test.tsx  ***REMOVED*** NEW
  triage-commit-copy.test.tsx        ***REMOVED*** NEW
```

---

***REMOVED******REMOVED*** Task 1: Scaffold `parquet-exporter.ts` (lazy DuckDB instance + tests)
...
***REMOVED******REMOVED*** Task 2: Implement `exportCanonicalToParquet` via DuckDB Appender API
...
***REMOVED******REMOVED*** Task 3: Add `GET /api/dimensions/:id/snapshot.parquet` endpoint
...
***REMOVED******REMOVED*** Task 4: Add `GET /api/workspace/info` endpoint
...
***REMOVED******REMOVED*** Task 5: Branch `commit()` in `repo-drafts.ts` for writable mode
...
***REMOVED******REMOVED*** Task 6: Frontend `useWorkspaceInfo` store hook
...
***REMOVED******REMOVED*** Task 7: Dashboard canonical-destination KPI
...
***REMOVED******REMOVED*** Task 8: Triage commit-affordance copy adapts to mode
...
***REMOVED******REMOVED*** Task 9: "Needs resync" badge per dimension in Dashboard
...
***REMOVED******REMOVED*** Task 10: "Download snapshot" link in MasterTables.tsx dimension header
...
***REMOVED******REMOVED*** Task 11: Verification gates
