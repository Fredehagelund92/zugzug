***REMOVED*** Phase 5 — Legal + scrub prep implementation plan
...
```
zugzug/
├── LICENSE                              ***REMOVED*** MIT, "<COPYRIGHT_HOLDER>" placeholder
├── NOTICE.md                            ***REMOVED*** Auto-generated dep attribution
├── CONTRIBUTING.md                      ***REMOVED*** DCO 1.1 + Signed-off-by convention
├── SECURITY.md                          ***REMOVED*** GHSA-only disclosure flow
├── CODE_OF_CONDUCT.md                   ***REMOVED*** Contributor Covenant 2.1 by reference
├── licenses.allowlist.json              ***REMOVED*** Escape hatch for license-checker exceptions
├── .gitleaksignore                      ***REMOVED*** Known-acceptable matches (test fixtures, etc.)
├── scripts/
│   ├── README.md                        ***REMOVED*** How to use each script + Phase 6 runbook
│   ├── check-license-placeholder.sh     ***REMOVED*** CI step; fails until <COPYRIGHT_HOLDER> swapped
│   ├── run-license-check.sh             ***REMOVED*** Wrapper around license-checker
│   ├── generate-notice.sh               ***REMOVED*** Regenerates NOTICE.md
│   ├── audit-history.sh                 ***REMOVED*** gitleaks + project-string grep (manual)
│   ├── scrub-history.sh                 ***REMOVED*** git-filter-repo (manual; refuses on live repo)
│   └── replacements.txt                 ***REMOVED*** Exact from===>to mappings for scrub
├── .github/workflows/ci.yml             ***REMOVED*** Adds license-check + placeholder-check + notice-up-to-date
└── ROADMAP.md                           ***REMOVED*** Updated: Phase 5 done; Phase 6 blockers listed
```

---

***REMOVED******REMOVED*** Verification gate (must all pass at end of phase)
...
***REMOVED******REMOVED*** Task 1: Legal documents (LICENSE, CONTRIBUTING, SECURITY, CODE_OF_CONDUCT)
...
***REMOVED******REMOVED******REMOVED*** Step 1: Create LICENSE
...
***REMOVED******REMOVED******REMOVED*** Step 2: Create CONTRIBUTING.md
...
***REMOVED******REMOVED******REMOVED*** Step 3: Create SECURITY.md

```markdown
***REMOVED*** Security policy

***REMOVED******REMOVED*** Reporting vulnerabilities
...
***REMOVED******REMOVED******REMOVED*** Step 4: Create CODE_OF_CONDUCT.md
...
***REMOVED******REMOVED*** Reporting concerns
...
***REMOVED******REMOVED******REMOVED*** Step 5: Verify + commit
...
***REMOVED******REMOVED*** Task 2: Placeholder safety check script + CI step

**Files:**
- Create: `scripts/check-license-placeholder.sh`
- Modify: `.github/workflows/ci.yml`

***REMOVED******REMOVED******REMOVED*** Step 1: Create the script
...
***REMOVED******REMOVED******REMOVED*** Step 2: Add CI step
...
***REMOVED******REMOVED******REMOVED*** Step 3: Verify locally
...
***REMOVED******REMOVED*** Task 3: license-checker CI gate per workspace
...
***REMOVED******REMOVED******REMOVED*** Step 1: Probe the actual dep license situation
...
***REMOVED******REMOVED******REMOVED*** Step 2: Create the allowlist file
...
***REMOVED******REMOVED******REMOVED*** Step 3: Create the wrapper script
...
***REMOVED******REMOVED******REMOVED*** Step 4: Verify locally
...
***REMOVED******REMOVED******REMOVED*** Step 5: Add CI steps
...
***REMOVED******REMOVED*** Task 4: NOTICE.md generator + initial snapshot + CI check
...
***REMOVED******REMOVED******REMOVED*** Step 1: Create the generator

`/Users/fhagelund/Documents/GitHub/zugzug/scripts/generate-notice.sh`:

```bash
***REMOVED***!/usr/bin/env bash
***REMOVED*** Regenerates NOTICE.md by listing all direct + transitive deps in both
***REMOVED*** workspaces with their license. Commit the resulting NOTICE.md alongside
***REMOVED*** code changes so it stays in sync.
...
generate_section() {
  local workspace="$1"
  echo "***REMOVED******REMOVED*** $workspace dependencies"
  echo ""
...
***REMOVED******REMOVED******REMOVED*** Step 2: Generate the initial NOTICE.md
...
***REMOVED******REMOVED******REMOVED*** Step 3: Add CI check that NOTICE.md stays current
...
***REMOVED******REMOVED*** Task 5: History audit (gitleaks + project-string grep)

**Files:**
- Create: `scripts/audit-history.sh`
- Create: `.gitleaksignore`

***REMOVED******REMOVED******REMOVED*** Step 1: Install gitleaks for local invocation
...
***REMOVED******REMOVED******REMOVED*** Step 2: Create `.gitleaksignore`

`/Users/fhagelund/Documents/GitHub/zugzug/.gitleaksignore`:

```
***REMOVED*** .gitleaksignore — one fingerprint per line. Each fingerprint allow-lists
***REMOVED*** a specific finding by its <commit-sha>:<path>:<rule-id>:<line-number> shape.
***REMOVED***
***REMOVED*** Add a comment above each entry explaining why it's safe to ignore.
***REMOVED*** Format: lines starting with ***REMOVED*** are comments.
***REMOVED***
***REMOVED*** Test fixtures with intentionally-fake tokens should be the only entries
***REMOVED*** here. Real leaked credentials must be revoked + rewritten, not ignored.
```

Start empty. Step 5 may add entries after the first audit run surfaces false positives.

***REMOVED******REMOVED******REMOVED*** Step 3: Create the audit script
...
***REMOVED******REMOVED******REMOVED*** Step 4: Run the audit
...
- [ ] Read the output. Expected:
  ...
  - repo_codename: depends — check if `zugzug` appears in history beyond CLAUDE.md
...
***REMOVED******REMOVED******REMOVED*** Step 5: Handle gitleaks findings (if any)
...
***REMOVED******REMOVED*** Task 6: Scrub script + replacements.txt + scripts/README
...
***REMOVED******REMOVED******REMOVED*** Step 1: Create replacements.txt
...
***REMOVED***
***REMOVED***
***REMOVED*** These mappings rewrite BC-internal strings to generic equivalents.
***REMOVED*** Adjust before Phase 6 if specific strings need different replacements.
...
Note the order: more-specific patterns first (`@example.com` before `example.com`) so the email-shaped form gets the email-shaped replacement.

The Sentry org URL pattern isn't here yet — Step 5 of Task 5's audit may surface specific URLs to add. If so, append entries like `sentry.io/REDACTED==>sentry.io/PROJECT_ID` per finding.

***REMOVED******REMOVED******REMOVED*** Step 2: Create the scrub script

`/Users/fhagelund/Documents/GitHub/zugzug/scripts/scrub-history.sh`:

```bash
***REMOVED***!/usr/bin/env bash
***REMOVED*** Rewrites git history to replace BC-internal strings per scripts/replacements.txt.
***REMOVED*** Uses git-filter-repo (the modern replacement for git-filter-branch).
***REMOVED***
***REMOVED*** SAFETY: This script refuses to run against the canonical Fredehagelund92/zugzug
***REMOVED*** repo. To run a scrub:
***REMOVED***   1. Clone the repo to a fresh working dir: git clone --mirror <upstream> mirror
***REMOVED***   2. cd mirror
***REMOVED***   3. Invoke this script
***REMOVED***   4. Inspect the result with scripts/audit-history.sh
***REMOVED***   5. Phase 6 only: git push --mirror to the new public destination
...
***REMOVED*** === Safety check: refuse to rewrite the live source-of-truth repo ===
...
***REMOVED******REMOVED******REMOVED*** Step 3: Verify the safety check works

- [ ] Run from the live repo: `bash scripts/scrub-history.sh`
- [ ] Confirm it exits 2 with the "REFUSING" message.

***REMOVED******REMOVED******REMOVED*** Step 4: Create scripts/README.md
...
***REMOVED******REMOVED*** Task 7: Dry-run the scrub against a mirror clone (verification)

This task produces no committed artifacts — it verifies the scrub script works end-to-end.

***REMOVED******REMOVED******REMOVED*** Step 1: Create the mirror clone
...
***REMOVED******REMOVED******REMOVED*** Step 2: Run the scrub
...
***REMOVED******REMOVED******REMOVED*** Step 3: Sanity-check the rewrite

- [ ] Run:
  ```bash
  git log --all -p --no-color | grep -iE '@bettercollective\.com|zugzug' | head -5
  ```
- [ ] Confirm zero matches.

***REMOVED******REMOVED******REMOVED*** Step 4: Clean up
...
***REMOVED******REMOVED*** Task 8: ROADMAP update

**Files:**
- Modify: `ROADMAP.md`

***REMOVED******REMOVED******REMOVED*** Step 1: Read the current state

- [ ] Run: `head -50 ROADMAP.md` to see the current section structure.

***REMOVED******REMOVED******REMOVED*** Step 2: Update the OSS-pivot section
...
+ ***REMOVED******REMOVED******REMOVED*** Phase 5 — Legal + scrub prep (DONE 2026-06-09)
...
+ ***REMOVED******REMOVED******REMOVED*** Phase 6 — Public push + v1.0 (BLOCKED — awaiting BC legal)
+
...
***REMOVED******REMOVED******REMOVED*** Step 3: Commit
...
***REMOVED******REMOVED*** Task 9: Verification gate run

This task is the final pass-or-iterate before shipping the PR.

***REMOVED******REMOVED******REMOVED*** Step 1: Run every gate item
...
***REMOVED******REMOVED******REMOVED*** Step 2: Push the branch + open PR
...
***REMOVED******REMOVED******REMOVED*** Step 3: Verify CI on the PR
