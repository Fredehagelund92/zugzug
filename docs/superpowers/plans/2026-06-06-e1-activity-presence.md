***REMOVED*** E1 — Activity & Presence Implementation Plan
...
***REMOVED******REMOVED*** Task 1: Migration — `audit_log.table_id`, `audit_log.row_key`, covering index
...
***REMOVED******REMOVED*** Task 2: Extend `appendAuditAs` to accept optional `{ tableId, rowKey }`
...
***REMOVED******REMOVED*** Task 3: Back-populate per-row audit call sites in `repo-canonical.ts`
...
***REMOVED******REMOVED*** Task 4: Per-row audit emission in `commit()` (`repo-drafts.ts`)
...
***REMOVED******REMOVED*** Task 5: `repo-activity.ts` — `getRowActivitySince` + unit test
...
***REMOVED******REMOVED*** Task 6: `GET /api/tables/:id/row-activity` route
...
***REMOVED******REMOVED*** Task 7: Client `use-row-activity.ts` hook
...
***REMOVED******REMOVED*** Task 8: `RowActivityBadge` component
...
***REMOVED******REMOVED*** Task 9: Wire `RowActivityBadge` into `DataGrid` row rendering
...
***REMOVED******REMOVED*** Task 10: Add `coral` / `sky` / `lime` tint tokens
...
```css
--tint-coral: ***REMOVED***e8523a;
--tint-sky:   ***REMOVED***2280d2;
--tint-lime:  ***REMOVED***6ab04c;

--tint-coral-fg: ***REMOVED***ffd9d0;
--tint-sky-fg:   ***REMOVED***cfe5f7;
--tint-lime-fg:  ***REMOVED***d8eecf;
...
```css
--tint-coral: ***REMOVED***c64432;
--tint-sky:   ***REMOVED***1b6ca8;
--tint-lime:  ***REMOVED***58963f;

--tint-coral-fg: ***REMOVED***7a2417;
--tint-sky-fg:   ***REMOVED***0f3f63;
--tint-lime-fg:  ***REMOVED***2f5722;
...
***REMOVED******REMOVED*** Task 11: `use-presence-color` hook + test
...
***REMOVED******REMOVED*** Task 12: Add yjs deps (server + client)
...
***REMOVED******REMOVED*** Task 13: `presence-room.ts` — `InMemoryPresenceTransport` + tests
...
***REMOVED******REMOVED*** Task 14: WebSocket upgrade branch in `server.ts`
...
***REMOVED******REMOVED*** Task 15: Client `use-presence` hook
...
***REMOVED******REMOVED*** Task 16: `CursorOverlay` component
...
***REMOVED******REMOVED*** Task 17: Wire `CursorOverlay` + `usePresence` into `DataGrid`
...
***REMOVED******REMOVED*** Task 18: `PresenceStrip` component
...
***REMOVED******REMOVED*** Task 19: Mount `PresenceStrip` in `TablePane` toolbar
...
***REMOVED******REMOVED*** Task 20: `row_touched` hint emission + client invalidation
...
***REMOVED******REMOVED*** Task 21: Manual smoke walkthrough
...
```bash
git log --oneline -25
***REMOVED*** Verify the 20 feature commits + this plan are all present.
```

---

***REMOVED******REMOVED*** Self-review notes
