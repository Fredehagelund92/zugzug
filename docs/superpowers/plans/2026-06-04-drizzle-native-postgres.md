# Drizzle + Native Postgres Architecture Implementation Plan
...
## Task 2: Create `server/src/pg.ts`
...
## Task 3: Create `server/drizzle/schema.ts`
...
## Task 4: Create `server/drizzle.config.ts` and `server/drizzle/migrate.ts`
...
## Task 5: Generate and Edit the Baseline Migration
...
## Task 6: Update `server/src/bootstrap.ts`
...
## Task 7: Update `server/src/db.ts` — Remove Postgres ATTACH
...
## Task 8: Update `server/src/env.ts` — Fix `pg()` Helper
...
## Task 9: Migrate `auth.ts` and `team.ts`
...
## Task 10: Update `repo.ts` — Imports and `cq()` Helper
...
## Task 11: Migrate Pure OLTP Functions in `repo.ts`
...
## Task 12: Migrate Dimension Registry + Canonical + `commit` in `repo.ts`
...
## Task 13: Decompose Cross-Store Join Functions
...
## Task 14: Migrate `getDimension` and `scanSources`
...
## Task 15: Delete `schema.ts` and Final Cleanup
