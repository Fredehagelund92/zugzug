...
## Task 1: Install `jose` + add auth env vars
...
# Authorized redirect URI must include: ORIGIN/api/auth/callback
GOOGLE_CLIENT_ID=
GOOGLE_CLIENT_SECRET=

# Domain restriction — only @this-domain.com emails may log in.
ALLOWED_DOMAIN=bettercollective.com

# Public base URL of this app. Used to build the OAuth redirect_uri.
# Dev: http://localhost:5173   Prod: https://yourapp.com
...
## Task 2: Schema migrations — `allowed_emails`, `sessions`, `users` columns
...
## Task 3: Create `server/src/auth.ts`
...
## Task 4: Wire auth into `server.ts` — session gate + route mounting
...
```bash
# Should redirect to Google (302 with Location header)
...
# Should return 401 for a protected route
...
## Task 5: Create `server/src/team.ts` + wire into `server.ts`
...
## Task 6: Create `app/src/routes/Login.tsx`
...
## Task 7: Update `BootGate` + `main.tsx`
...
## Task 8: Update `store.ts` — drop `x-user-id`, add `email` to `User`
...
## Task 9: Update `AppShell` — real user display + sign out
...
## Task 10: Add Team section to `Settings.tsx`
...
Log in with a non-`@bettercollective.com` Google account. Expected: `/login?error=domain` with the domain error message.
