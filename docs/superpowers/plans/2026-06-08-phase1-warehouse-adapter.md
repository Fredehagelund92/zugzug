***REMOVED*** Phase 1 — Extract `WarehouseAdapter` against DuckDB only
...
server/src/
  warehouse/
    adapter.ts          ***REMOVED*** interface, types, Ref, AdapterCapabilities, isWritable
    credentials.ts      ***REMOVED*** Zod discriminated union + factory registry
    registry.ts         ***REMOVED*** getAdapter() with per-key cache
    duckdb/
      index.ts          ***REMOVED*** DuckDbAdapter — owns the @duckdb/node-api connection
    snowflake/
      index.ts          ***REMOVED*** SnowflakeAdapter stub (throws — implemented in Phase 2)
  db.ts                 ***REMOVED*** SHRUNK: only the connection lifecycle for DuckDbAdapter to use internally
  repo-scan.ts          ***REMOVED*** uses adapter; no raw DuckDB
  repo-canonical.ts     ***REMOVED*** uses adapter; no raw DuckDB
  repo-drafts.ts        ***REMOVED*** uses adapter; no raw DuckDB
  repo-shared.ts        ***REMOVED*** whTable/occUnion deleted; all/get/run re-exports removed
  ...
server/test/
  warehouse-adapter.test.ts   ***REMOVED*** NEW — adapter contract tests against in-memory DuckDB
  ... (existing tests unchanged; they keep ATTACH_WAREHOUSE=false)
```

---

***REMOVED******REMOVED*** Task 1: Add Zod dependency
...
***REMOVED******REMOVED*** Task 2: Scaffold `warehouse/adapter.ts` — interface + types
...
***REMOVED******REMOVED*** Task 3: Scaffold `warehouse/credentials.ts` — Zod schemas + factory registry
...
***REMOVED******REMOVED*** Task 4: Scaffold `warehouse/registry.ts` — `getAdapter(workspaceId)` with cache
...
***REMOVED******REMOVED*** Task 5: Implement `DuckDbAdapter` — helpers + capabilities + `ping()`
...
***REMOVED******REMOVED*** Task 6: Implement `DuckDbAdapter` — all read methods
...
***REMOVED******REMOVED*** Task 7: Refactor `liveSources` in `repo-shared.ts` to use the adapter
...
***REMOVED******REMOVED*** Task 8: Refactor `repo-scan.ts:scanSources` to use the adapter
...
***REMOVED******REMOVED*** Task 9: Refactor `repo-scan.ts:topUnmapped` + `autoStageExactMatches` + `searchCatalog`
...
***REMOVED******REMOVED*** Task 10: Refactor `repo-canonical.ts` — name resolution + multi-source scans
...
***REMOVED******REMOVED*** Task 11: Refactor `repo-drafts.ts:rowsRecovered` query (the last `occUnion` user)
...
***REMOVED******REMOVED*** Task 12: Delete `whTable`, `occUnion`, and DuckDB re-exports from `repo-shared.ts`
...
***REMOVED******REMOVED*** Task 12b: Replace `connect()` in `server.ts` and `bootstrap.ts` with adapter ping
...
***REMOVED******REMOVED*** Task 13: Stub `SnowflakeAdapter`
...
***REMOVED******REMOVED*** Task 14: Verification gates
