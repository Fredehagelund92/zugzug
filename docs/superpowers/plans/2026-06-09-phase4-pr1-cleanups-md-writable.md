***REMOVED*** Phase 4 PR 1 — Cleanups + MotherDuck-writable Implementation Plan
...
9. `grep -rn "Zugzug\|bettercollective" server/src/ app/src/ 2>&1 | grep -v "test/" | grep -v "node_modules"` — zero matches (except possibly inline comments calling out the historical context, which are fine).
...
```
server/
  .env.example                                  ***REMOVED*** MODIFIED — adds MOTHERDUCK_WRITABLE, DEFAULT_ENGINEER_MODE
server/src/
  env.ts                                        ***REMOVED*** MODIFIED — adds motherduckWritable, defaultEngineerMode
  seed.ts                                       ***REMOVED*** REWRITTEN — generic e-commerce
  warehouse/credentials.ts                      ***REMOVED*** MODIFIED — adds writable?: boolean to DuckDbCredentials
  warehouse/registry.ts                         ***REMOVED*** MODIFIED — reads env.motherduckWritable
  warehouse/duckdb/
    index.ts                                    ***REMOVED*** MODIFIED — now barrel-exports the two new classes + retains base
    base.ts                                     ***REMOVED*** NEW — shared connection/serialization/query helpers
    read-only.ts                                ***REMOVED*** NEW — DuckDbReadOnlyAdapter (current shape, refactored to use base)
    writable.ts                                 ***REMOVED*** NEW — DuckDbWritableAdapter (extends base + write methods)
  server.ts                                     ***REMOVED*** MODIFIED — workspace/info adds defaultEngineerMode + allowedDomain; factory uses creds.writable
  bootstrap.ts                                  ***REMOVED*** MODIFIED — same factory split
server/test/
  warehouse-duckdb.test.ts                      ***REMOVED*** MODIFIED — split into RO + Writable describe blocks; writable tests
  commit-warehouse-branch.test.ts               ***REMOVED*** MODIFIED — adds DuckDB-writable case alongside Snowflake mock
app/src/
  store.ts                                      ***REMOVED*** MODIFIED — WorkspaceInfo gains defaultEngineerMode + allowedDomain; validator updated
  lib/engineer-mode.tsx                         ***REMOVED*** MODIFIED — reads server default when no localStorage; null-safe initial state
  routes/Login.tsx                              ***REMOVED*** MODIFIED — copy scrub (remove "Zugzug"); allowedDomain derived
  routes/Settings.tsx                           ***REMOVED*** MODIFIED — ALLOWED_DOMAIN derived from useWorkspaceInfo; placeholder text generic
app/test/
  engineer-mode-default.test.tsx                ***REMOVED*** NEW — server default applied when no localStorage
  login-copy.test.tsx                           ***REMOVED*** NEW — copy scrub assertions
```

---

***REMOVED******REMOVED*** Task 1: Add `MOTHERDUCK_WRITABLE` and `DEFAULT_ENGINEER_MODE` env vars
...
```bash
***REMOVED*** MotherDuck-writable mode — flip to "true" only if your MotherDuck token has
***REMOVED*** write access. When true, commits write dim_*/map_* records to your MotherDuck
***REMOVED*** database (canonical destination = warehouse). When false (default), canonical
***REMOVED*** lives in Postgres and snapshots are downloadable on demand.
MOTHERDUCK_WRITABLE=false

***REMOVED*** Engineer-mode default — when "false", hides warehouse internals (table names,
***REMOVED*** SQL, MERGE/JOIN copy) for non-technical analysts. Each user can override this
***REMOVED*** in Settings. Default: true (engineer details shown).
...
***REMOVED******REMOVED*** Task 2: Add `writable` to `DuckDbCredentials` Zod schema
...
***REMOVED******REMOVED*** Task 3: Update `registry.ts` to wire `MOTHERDUCK_WRITABLE` env into credentials
...
***REMOVED******REMOVED*** Task 4: Extract `DuckDbAdapter` shared logic into a base class
...
***REMOVED******REMOVED*** Task 5: Split `DuckDbAdapter` into `DuckDbReadOnlyAdapter` + `DuckDbWritableAdapter`
...
***REMOVED******REMOVED*** Task 6: Implement `DuckDbWritableAdapter.ensureCanonicalTables`
...
***REMOVED******REMOVED*** Task 7: Implement `DuckDbWritableAdapter.commitCanonical` via chunked MERGE INTO
...
***REMOVED******REMOVED*** Task 8: Extend `/api/workspace/info` with `defaultEngineerMode` + `allowedDomain`
...
***REMOVED******REMOVED*** Task 9: Engineer-mode default flip — use server default when no localStorage
...
***REMOVED******REMOVED*** Task 10: Seed-data scrub — generic e-commerce dimensions
...
***REMOVED******REMOVED*** Task 11: UI copy scrub — Login.tsx and Settings.tsx
...
Replace hard-coded "Zugzug" / "example.com" strings. The server's `env.allowedDomain` default changes from `"example.com"` to `""`; the frontend reads it live via `useWorkspaceInfo().allowedDomain` (added in Task 8).
...
```
Expected: "does not mention Zugzug" FAILS (current Login.tsx says "Master data reconciliation · Zugzug." and has a `example.com` error message).
...
Replace the placeholder text at line ~462 (search for "colleague@example.com"):
...
git commit -m "chore(ui): remove Zugzug copy; allowedDomain now env-driven"
```

---

***REMOVED******REMOVED*** Task 12: Extend `commit-warehouse-branch.test.ts` with DuckDB-writable case
...
***REMOVED******REMOVED*** Task 13: Verification gates

**Files:** none modified.

- [ ] **Step 1: Grep gate — no "Zugzug" / "bettercollective" in production code**

```bash
grep -rn "Zugzug\|bettercollective" /Users/fhagelund/Documents/GitHub/zugzug/server/src/ /Users/fhagelund/Documents/GitHub/zugzug/app/src/ 2>&1 | grep -v "test/" | head -20
```
...
Verify:
1. Login page shows "Master data reconciliation." (no "Zugzug" text).
...
3. With `MOTHERDUCK_WRITABLE=true` (restart server first): Settings → Connections → Master records shows "Saved to Duckdb"; commit affordance in Triage reads "Approve & commit to warehouse"; Dashboard topbar (if you re-added it from a previous iteration — chip was removed in PR ***REMOVED***84) — N/A; we removed the topbar chip already.
...
- `chore(ui)`: remove Zugzug copy; allowedDomain now env-driven
