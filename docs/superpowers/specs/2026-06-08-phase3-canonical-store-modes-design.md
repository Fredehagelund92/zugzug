***REMOVED*** Phase 3 — Canonical-store modes (design spec)
...
***REMOVED******REMOVED*** What Phase 3 is NOT
...
***REMOVED******REMOVED*** Strategic decisions (locked during 2026-06-08 brainstorming)
...
***REMOVED******REMOVED******REMOVED*** Module structure (post-phase)

```
server/src/warehouse/
  parquet-exporter.ts          ***REMOVED*** NEW — standalone Parquet writer (uses an always-in-process DuckDB)
  registry.ts                  ***REMOVED*** unchanged
  adapter.ts                   ***REMOVED*** unchanged (the spec-leak comment about exportCanonicalSnapshot stays, since we're moving it OUT of the adapter; might update the comment text)
  duckdb/index.ts              ***REMOVED*** unchanged
  snowflake/index.ts           ***REMOVED*** unchanged
server/src/
  repo-drafts.ts               ***REMOVED*** commit() branches on isWritable(adapter)
  server.ts                    ***REMOVED*** adds GET /api/dimensions/:id/snapshot.parquet + GET /api/workspace/info routes
app/src/
  routes/Dashboard.tsx         ***REMOVED*** adds canonical-destination badge + "needs resync" hint per dimension
  routes/Triage.tsx            ***REMOVED*** commit-affordance copy per mode
  store.ts                     ***REMOVED*** adds workspace.info fetch + sync-status derived state
```

***REMOVED******REMOVED******REMOVED*** Interface change: `ReadOnlyWarehouseAdapter`
...
***REMOVED******REMOVED******REMOVED*** `parquet-exporter.ts` — the standalone utility
...
***REMOVED******REMOVED******REMOVED*** `commit()` branching in `repo-drafts.ts`
...
***REMOVED******REMOVED******REMOVED*** Audit events
...
***REMOVED******REMOVED******REMOVED*** Snapshot endpoint
...
***REMOVED******REMOVED******REMOVED*** `/api/workspace/info` endpoint
...
***REMOVED******REMOVED******REMOVED*** Triage (commit affordance)
...
***REMOVED******REMOVED******REMOVED*** Dimension detail (Tables.tsx)
...
***REMOVED******REMOVED******REMOVED*** Engineer-mode gating
...
***REMOVED******REMOVED******REMOVED*** Writable-warehouse commit
```
...
***REMOVED******REMOVED******REMOVED*** Read-only-warehouse commit (current default)
```
...
***REMOVED******REMOVED******REMOVED*** Snapshot download
```
...
***REMOVED******REMOVED*** Out of scope (deferred to Phase 4 or v1.1+)
