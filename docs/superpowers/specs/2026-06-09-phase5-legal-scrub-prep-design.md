...
2. Manual grep over full history for the project-specific strings (documented inline in the script for transparency):
   ...
   - `Better Collective` (case-insensitive, as a phrase)
   - `trust-me-bro`
...
- **`<COPYRIGHT_HOLDER>` value** — pending BC legal call (`Better Collective A/S and contributors` vs `Frederik Hagelund and contributors` vs both).
...
- **`trust-me-bro` removal scope** — currently grep-matches in CLAUDE.md (the codename is documented). Confirm we want every occurrence rewritten to `zugzug` (or removed) — including in our own internal-process docs.
