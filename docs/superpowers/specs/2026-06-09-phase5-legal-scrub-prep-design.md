***REMOVED*** Phase 5 — Legal + scrub prep (design)
...
├── NOTICE.md                            ***REMOVED*** Auto-generated from license-checker
├── CONTRIBUTING.md                      ***REMOVED*** DCO 1.1 + Signed-off-by
├── SECURITY.md                          ***REMOVED*** Points to GHSA only
├── CODE_OF_CONDUCT.md                   ***REMOVED*** Contributor Covenant 2.1
├── licenses.allowlist.json              ***REMOVED*** Escape hatch for legitimate exceptions
├── .gitleaksignore                      ***REMOVED*** Known-acceptable matches (e.g., test fixtures)
├── scripts/
│   ├── check-license-placeholder.sh     ***REMOVED*** CI step; fails until placeholder swapped
│   ├── audit-history.sh                 ***REMOVED*** Manual: gitleaks + project-string grep
│   ├── scrub-history.sh                 ***REMOVED*** Manual: git-filter-repo (refuses on tracked repo)
│   ├── replacements.txt                 ***REMOVED*** Exact from===>to mappings for the scrub
│   ├── generate-notice.sh               ***REMOVED*** Regenerates NOTICE.md from license-checker
│   └── README.md                        ***REMOVED*** How to invoke each script + Phase 6 runbook
└── .github/workflows/
    └── ci.yml                           ***REMOVED*** Adds license-checker steps + placeholder check
```

***REMOVED******REMOVED*** Design decisions (locked)

***REMOVED******REMOVED******REMOVED*** Legal documents
...
***REMOVED******REMOVED******REMOVED*** Placeholder safety net
...
***REMOVED******REMOVED******REMOVED*** license-checker CI gate
...
***REMOVED******REMOVED******REMOVED*** History audit
...
***REMOVED******REMOVED******REMOVED*** History scrub (dry-run-able, not executed in Phase 5)
...
```
***REMOVED*** git-filter-repo --replace-text format: <pattern>==><replacement>
***REMOVED*** One pattern per line. Lines starting with ***REMOVED*** are comments.
...
```bash
***REMOVED*** Phase 6 runbook (DO NOT RUN IN PHASE 5):
git clone --mirror https://github.com/Fredehagelund92/zugzug zugzug-mirror
cd zugzug-mirror
../scripts/scrub-history.sh
***REMOVED*** Verify the audit passes:
../scripts/audit-history.sh
***REMOVED*** Then point at the new public remote and force-push:
git remote set-url origin <PUBLIC_REPO_URL>
git push --mirror
```

***REMOVED******REMOVED******REMOVED*** ROADMAP update
...
***REMOVED******REMOVED*** Gate (must pass at end of Phase 5)
...
***REMOVED******REMOVED*** Open questions to resolve before Phase 6 starts
...
***REMOVED******REMOVED*** Out of scope (deferred to Phase 6 or v1.1+)
