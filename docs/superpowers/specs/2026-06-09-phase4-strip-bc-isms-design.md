***REMOVED*** Phase 4 — Strip BC-isms + MotherDuck-writable + Auth refactor (design spec)
...
***REMOVED******REMOVED*** Strategic decisions (locked during 2026-06-09 brainstorming)
...
***REMOVED******REMOVED******REMOVED*** Two-PR split (Approach B from brainstorming)
...
```
***REMOVED*** PR 1: Cleanups + MD-writable
server/src/
  warehouse/duckdb/index.ts         ***REMOVED*** MODIFIED — splits into RO and Writable variants OR polymorphic single class
  warehouse/credentials.ts          ***REMOVED*** MODIFIED — adds writable?: boolean to DuckDbCredentials
  warehouse/registry.ts             ***REMOVED*** MODIFIED — reads MOTHERDUCK_WRITABLE env
  env.ts                            ***REMOVED*** MODIFIED — adds motherduckWritable, defaultEngineerMode flags
  seed.ts                           ***REMOVED*** REWRITTEN — generic e-commerce dimensions
app/src/
  lib/engineer-mode.tsx             ***REMOVED*** MODIFIED — default flips ON; reads server-provided default
  routes/Login.tsx                  ***REMOVED*** MODIFIED — copy scrub ("Zugzug" → generic)
  routes/Settings.tsx               ***REMOVED*** MODIFIED — copy scrub (allowed-domain placeholder text)
server/test/
  warehouse-duckdb.test.ts          ***REMOVED*** MODIFIED — adds writable-mode commit tests
  commit-warehouse-branch.test.ts   ***REMOVED*** MODIFIED — adds DuckDB-writable case alongside Snowflake mock

***REMOVED*** PR 2: Auth + API tokens
server/drizzle/
  schema.ts                         ***REMOVED*** MODIFIED — password_hash + auth_provider on users; new api_tokens table
  migrations/000N_auth_refactor.sql ***REMOVED*** GENERATED via `bun run db:generate`
server/src/
  auth.ts                           ***REMOVED*** REWRITTEN — generic OIDC + password endpoints
  auth-oidc.ts                      ***REMOVED*** NEW — openid-client wrapper (extracted for testability)
  auth-password.ts                  ***REMOVED*** NEW — argon2 hashing + login/signup handlers
  auth-api-tokens.ts                ***REMOVED*** NEW — token issuance/verification/revocation
  server.ts                         ***REMOVED*** MODIFIED — session gate falls back to bearer-token auth
  env.ts                            ***REMOVED*** MODIFIED — adds OIDC_* env vars; deprecates GOOGLE_*
  bootstrap.ts                      ***REMOVED*** MODIFIED — first-user-as-admin bootstrap on empty users table
scripts/
  reset-password.ts                 ***REMOVED*** NEW — CLI for admin password reset (Bun script)
app/src/
  routes/Login.tsx                  ***REMOVED*** REWRITTEN — fetches /api/auth/config, renders password form OR SSO button
  routes/Settings.tsx               ***REMOVED*** MODIFIED — new "API tokens" section with create/revoke
  store.ts                          ***REMOVED*** MODIFIED — adds useAuthConfig + token-management actions
server/test/
  auth-password.test.ts             ***REMOVED*** NEW — signup, login, change-password, allowlist, password complexity
  auth-oidc.test.ts                 ***REMOVED*** NEW — OIDC callback with mocked openid-client
  auth-api-tokens.test.ts           ***REMOVED*** NEW — issuance, hash uniqueness, bearer-token auth, revocation
app/test/
  login-mode-aware.test.tsx         ***REMOVED*** NEW — Login renders right UI based on config mode
  api-tokens-settings.test.tsx      ***REMOVED*** NEW — token list, create, revoke flows
```

---

***REMOVED******REMOVED*** PR 1 details

***REMOVED******REMOVED******REMOVED*** Engineer-mode default flip
...
***REMOVED******REMOVED******REMOVED*** Seed-data scrub
...
***REMOVED******REMOVED******REMOVED*** UI copy scrub

| File:line | Before | After |
|---|---|---|
| `server/src/env.ts:30` | `allowedDomain: ... \|\| "example.com"` | `allowedDomain: ... \|\| ""` (empty = unrestricted; comment: "set OIDC_ALLOWED_DOMAIN to restrict signups") |
| `app/src/routes/Login.tsx:5` | `domain: "Only @example.com accounts can access this app."` | Generic — derived from `/api/auth/config`'s `allowedDomain` field: `Only @${domain} accounts can access this app.` (or generic "This domain is restricted" when no domain set) |
| `app/src/routes/Login.tsx:44` | `Master data reconciliation · Zugzug.` | `Master data reconciliation.` |
| `app/src/routes/Settings.tsx:199` | `const ALLOWED_DOMAIN = "@example.com"` | derived from `/api/auth/config`; passed via context or a `useAllowedDomain()` hook |
| `app/src/routes/Settings.tsx:462` | placeholder `colleague@example.com, another@example.com…` | derived placeholder using the configured domain, or generic `colleague@example.com, another@example.com…` when unset |

***REMOVED******REMOVED******REMOVED*** MotherDuck-writable adapter
...
***REMOVED******REMOVED*** PR 2 details

***REMOVED******REMOVED******REMOVED*** Schema migration
...
***REMOVED******REMOVED******REMOVED*** Auth mode resolution
...
***REMOVED******REMOVED******REMOVED*** Password mode flow
...
***REMOVED******REMOVED******REMOVED*** OIDC mode flow
...
***REMOVED******REMOVED******REMOVED*** API tokens
...
***REMOVED******REMOVED******REMOVED*** Login.tsx rewrite
...
***REMOVED******REMOVED******REMOVED*** Settings.tsx — new "API tokens" section
...
***REMOVED*** OIDC_LABEL=Google (optional; defaults to issuer hostname)

***REMOVED*** Deprecated (can remove after migration verified):
***REMOVED*** GOOGLE_CLIENT_ID=...
***REMOVED*** GOOGLE_CLIENT_SECRET=...
***REMOVED*** ALLOWED_DOMAIN=...
```
...
***REMOVED******REMOVED*** Out of scope (deferred to v1.1+)
...
***REMOVED******REMOVED*** Migration / rollout
