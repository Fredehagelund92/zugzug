...
Reposition Zugzug from a Better Collective internal tool into an open-source master-data reconciliation product, while keeping BC's running instance functional throughout.
...
server/src/warehouse/
  adapter.ts          # interface + types + isWritable guard
  registry.ts         # getAdapter(workspaceId) → cached WarehouseAdapter
  credentials.ts      # Zod discriminated union + factory registry
  duckdb/index.ts     # DuckDbAdapter (covers DuckDB + MotherDuck)
  snowflake/index.ts  # SnowflakeAdapter
```
...
| BC jargon in UI copy | Grep + sweep; remove "Better Collective," "BC," internal hostnames, internal-team-name tooltips |
...
### Phase 1 — Extract `WarehouseAdapter` against DuckDB only (~weeks 1–3)
...
### Phase 2 — Implement `SnowflakeAdapter` (~weeks 4–6) — **gating phase**
...
### Phase 3 — Canonical-store modes (~weeks 7–8)
...
**Gate:** `git log --all -p | grep -iE '(token|secret|sentry|@bettercollective|trust-me-bro)'` returns nothing. License audit clean. BC legal sign-off in writing.
...
- Specific MIT copyright holder: "Better Collective A/S and contributors" vs "Frederik Hagelund and contributors" — BC legal call.
...
- #59 multi-tenant workspaces (phase 1) — multi-tenant UI work specifically; the schema/middleware work already done is preserved and gated behind `ZUGZUG_MULTI_TENANT`
- #45 canonical_history / per-record revert
- #33 bulk operations in Triage
