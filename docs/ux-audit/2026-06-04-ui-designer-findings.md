...
***REMOVED******REMOVED*** 1. Visual Diagnosis
...
***REMOVED******REMOVED*** 2. Design System Gaps

***REMOVED******REMOVED******REMOVED*** 2a. Color token gaps
...
Proposal:
```
dark:  --surface-elevated: ***REMOVED***1E2840   (sits above --surface-2: ***REMOVED***161E30, below --surface-3)
light: --surface-elevated: ***REMOVED***FFFFFF   (same as --surface in light)
```
...
|---|---|---|---|
| `--bg` | `***REMOVED***080B14` | `***REMOVED***080B14` | unchanged |
| `--surface` | `***REMOVED***0E1422` | `***REMOVED***0F1526` | +1 step |
| `--surface-2` | `***REMOVED***161E30` | `***REMOVED***162035` | widen gap |
| `--surface-3` | `***REMOVED***202B44` | `***REMOVED***1E2840` | pull back, less blue-shifted |
| `--surface-elevated` | (missing) | `***REMOVED***252E48` | new — for menus/popovers |
...
- `--committed` / `--committed-soft`: a distinct "published" state that is differentiated from the in-progress "mapped draft". Proposal: use a cooler teal-adjacent green rather than the warm `--ak-ok: ***REMOVED***1F9D6B`. E.g. `--committed: ***REMOVED***0E9F8A`, `--committed-soft: color-mix(in srgb, ***REMOVED***0E9F8A 12%, transparent)`.
- `--staged` / `--staged-soft`: amber that reads distinctly from `--ak-warn` (which the app also uses for "needs review" badges). Currently both the "this value has a pending draft" state and the "this dimension needs review" state use `warn` tone — they should diverge. Proposal: keep `warn` for attention/alerting, introduce `--staged: ***REMOVED***C47C18` (slightly more saturated ochre) for in-flight workflow state.

**The accent-2 / amber (***REMOVED***F0A323 dark / ***REMOVED***C68410 light) is used in exactly one place** (the `★` suggestion indicator in ComboSelect and the bottom-left corner glow on the canvas). This token earns its keep on the canvas atmospheric gradient but the `★` usage in the dropdown is tiny and hard to read at 12.5px. Either increase its usage to become a true secondary accent or replace the `★` with the existing `--accent-soft` wash.

***REMOVED******REMOVED******REMOVED*** 2b. Typography scale gaps
...
***REMOVED******REMOVED******REMOVED*** 2c. Spacing and density
...
***REMOVED******REMOVED******REMOVED*** 2d. Elevation / shadow scale
...
***REMOVED******REMOVED******REMOVED*** 2e. Motion scale
...
***REMOVED******REMOVED*** 3. Per-Screen Findings

***REMOVED******REMOVED******REMOVED*** 3a. Login screen
...
***REMOVED******REMOVED******REMOVED*** 3b. App shell (sidebar + topbar)
...
***REMOVED******REMOVED******REMOVED*** 3c. Dashboard
...
- **Activity feed timestamps** (`text-[10px] text-ink-3`) are too small and too faint. 10px text at `--ink-3: ***REMOVED***5B6884` fails WCAG AA on `--surface: ***REMOVED***0E1422` (contrast ratio ~3.0, needs 4.5 for small text). The timestamps should be at least 11px.
...
***REMOVED******REMOVED******REMOVED*** 3d. Mapping workbench
...
- **"nothing to publish yet" footer state**: the mono text at 11px at `text-ink-3` fails the WCAG AA contrast threshold on `--surface: ***REMOVED***0E1422`. Use `text-ink-2` for footer idle states.
...
***REMOVED******REMOVED******REMOVED*** 3e. Tables (MasterTables)
...
***REMOVED******REMOVED******REMOVED*** 3f. Sources
...
***REMOVED******REMOVED******REMOVED*** 3g. Settings
...
***REMOVED******REMOVED******REMOVED*** 3h. Datagrid column-header menu
...
Line 125: `delete` button uses `bg-danger text-white` — correct, but `text-white` is a literal color, not a token. Use `text-accent-ink` (which is `***REMOVED***fff` for danger's contrast requirements) or add a `--danger-ink: ***REMOVED***fff` token.
...
***REMOVED******REMOVED******REMOVED*** 3i. ComboSelect
...
***REMOVED******REMOVED*** 4. Interaction and Motion

***REMOVED******REMOVED******REMOVED*** 4a. Missing microinteractions
...
***REMOVED******REMOVED******REMOVED*** 4b. Missing loading states
...
***REMOVED******REMOVED******REMOVED*** 4c. Optimistic UI gaps
...
***REMOVED******REMOVED*** 5. Brand Direction Options
...
***REMOVED******REMOVED******REMOVED*** Option A: "Warm Editorial — The Data Newspaper"
...
- Change the background from `***REMOVED***080B14` to `***REMOVED***0A0D17` (cooler, more neutral — removes the blue cast). Surface steps become warmer: `--surface: ***REMOVED***111520` (warm-tinted neutral).
- Replace the accent `***REMOVED***D6336C` with `***REMOVED***E04F25` (a refined rust/terracotta) — warmer, less nightclub.
...
***REMOVED******REMOVED******REMOVED*** Option B: "Linear-Precise — The Engineer's Console"
...
***REMOVED******REMOVED******REMOVED*** Option C: "Data-Dense Engineer-First — The Terminal Elevated"
...
- Change the surface palette to be warmer and slightly more saturated: `--bg: ***REMOVED***0C0E16`, `--surface: ***REMOVED***11141F`, `--surface-2: ***REMOVED***171B2B`. This is a more "OLED-warm" space.
...
Current: `--surface: ***REMOVED***0E1422`, `--surface-2: ***REMOVED***161E30` (delta too small)  
Proposed: Increase surface-2 to `***REMOVED***172035` and add `--surface-elevated: ***REMOVED***232C44` for menus/popovers. Update both `:root` (dark) and `[data-theme="light"]` blocks.  
...
Locations:  
- `Dashboard.tsx:137`: `text-[10px] text-ink-3` timestamps — `***REMOVED***5B6884` on `***REMOVED***0E1422` ≈ 3.0:1 (fails AA small text 4.5:1)  
- `Mapping.tsx:732`: idle footer `text-ink-3` at 11px — same contrast failure  
- `AppShell.tsx:181`: `text-[11px] text-ink` Connected label — passes at `--ink: ***REMOVED***EAEEF7`, but the dot beside it is `bg-accent` at 6×6px = too small to count as meaningful indicator without a text label  
