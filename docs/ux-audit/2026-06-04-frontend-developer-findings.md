***REMOVED*** Frontend UX Audit — Zug Zug
**Date:** 2026-06-04  
...
***REMOVED******REMOVED*** 2. Critical Findings

***REMOVED******REMOVED******REMOVED*** CF-1: Dashboard crashes when dims is empty
...
***REMOVED******REMOVED******REMOVED*** CF-2: Dashboard KPI cards show hardcoded fixture data
...
***REMOVED******REMOVED******REMOVED*** CF-3: "Save changes" button on Settings does nothing for non-threshold fields
...
***REMOVED******REMOVED******REMOVED*** CF-4: Bulk undo is N sequential network round-trips, not one compound action
...
***REMOVED******REMOVED******REMOVED*** CF-5: ComboSelect has no keyboard-navigable option list
...
***REMOVED******REMOVED******REMOVED*** CF-6: No per-value or per-decision deep link
...
***REMOVED******REMOVED******REMOVED*** CF-7: Zero first-run / onboarding experience
...
***REMOVED******REMOVED*** 3. High-Impact Polish

***REMOVED******REMOVED******REMOVED*** P-1: `confirm()` dialog for type-change coerce path
...
***REMOVED******REMOVED******REMOVED*** P-2: Column header menu uses emoji for icons instead of the existing icon system
...
***REMOVED******REMOVED******REMOVED*** P-3: Mapping workbench "Record" ComboSelect trigger width is unconstrained
...
***REMOVED******REMOVED******REMOVED*** P-4: Review panel in commit footer is a flat list with no grouping
...
***REMOVED******REMOVED******REMOVED*** P-5: "Auto-match new values" button gives no undo feedback
...
***REMOVED******REMOVED******REMOVED*** P-6: MasterTables "Tip" toolbar hint is visible even with 1 record
...
***REMOVED******REMOVED******REMOVED*** P-7: Sources "standing callout" accent-left border competes with the overall page accent
...
***REMOVED******REMOVED******REMOVED*** P-8: Login page error handling misses 401 → already-authed path
...
***REMOVED******REMOVED*** 4. Workflow Improvements

***REMOVED******REMOVED******REMOVED*** W-1: No cmd-K command palette / quick-switcher
...
***REMOVED******REMOVED******REMOVED*** W-2: Undo stack is per-page-mount, cleared on dimension switch
...
***REMOVED******REMOVED******REMOVED*** W-3: Cross-dim inbox lacks "M" (manual pick) keyboard shortcut
...
***REMOVED******REMOVED******REMOVED*** W-4: No way to see all pending drafts across dimensions from a single view
...
***REMOVED******REMOVED******REMOVED*** W-5: No drag-to-map or multi-row drag affordance
...
***REMOVED******REMOVED******REMOVED*** W-6: Retire canonical record has no confirmation and fires immediately
...
***REMOVED******REMOVED*** 5. Code-Level Technical Debt Impacting UX

***REMOVED******REMOVED******REMOVED*** T-1: `initStore()` is a sequential waterfall of 5+ API calls
...
***REMOVED******REMOVED******REMOVED*** T-2: `refreshDims()` fetches every dimension individually
...
***REMOVED******REMOVED******REMOVED*** T-3: Global store uses a single emit/subscribe model — every mutation re-renders everything
...
***REMOVED******REMOVED******REMOVED*** T-4: The UndoStack `undo.undo()` error handling silently no-ops
...
***REMOVED******REMOVED******REMOVED*** T-5: `MasterTables` columns `useMemo` depends on `open` (provenance drawer state)
...
***REMOVED******REMOVED******REMOVED*** T-6: Settings `TeamSection.load()` has no error state
...
***REMOVED******REMOVED*** 6. What's Already Great
