***REMOVED*** Zug Zug — Merged UX Punch-List
**Date:** 2026-06-04
...
***REMOVED******REMOVED*** TL;DR — the shape of the work
...
***REMOVED******REMOVED*** Tier 0 — Ship first: trust-eroding bugs (<1 day total)

| ***REMOVED*** | Finding | Location | Effort |
|---|---------|----------|--------|
...
| 0.3 | **Settings "Save changes" is a no-op** — `save()` only sets a 2.2s flash; the workspace name input is uncontrolled and never sent anywhere. Domain restriction `@example.com` is also hardcoded. | `Settings.tsx:131`, `TeamSection.tsx:65` | S (remove phantom button + persist on blur) / M (real workspace endpoint) |
...
***REMOVED******REMOVED*** Tier 1 — Cross-confirmed quick wins (both audits agreed; <1 day each)

These are the highest-confidence visible improvements. Order is impact-to-effort.

***REMOVED******REMOVED******REMOVED*** 1.1 Column-header menu: kill the Unicode glyphs — **HIGH**
...
***REMOVED******REMOVED******REMOVED*** 1.2 ComboSelect: keyboard nav + focus restore — **HIGH**
...
***REMOVED******REMOVED******REMOVED*** 1.3 Review panel: bound the height — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 1.4 WCAG contrast pass on small text — **HIGH (accessibility)**
`text-ink-3` (`***REMOVED***5B6884`) on `--surface` (`***REMOVED***0E1422`) is ~3.0:1 at 10–11px — fails AA (needs 4.5:1). Hits Dashboard activity timestamps, Mapping idle footer, and several secondary labels. Blanket rule: **`text-ink-3` only on text ≥13px**; everything smaller goes to `text-ink-2`. **Effort: S (~30 min audit + swap).**

***REMOVED******REMOVED******REMOVED*** 1.5 Surface ramp: widen the gap, add `--surface-elevated` — **HIGH**
...
```
--surface:           ***REMOVED***0E1422 → ***REMOVED***0F1526
--surface-2:         ***REMOVED***161E30 → ***REMOVED***162035
--surface-3:         ***REMOVED***202B44 → ***REMOVED***1E2840
--surface-elevated:  (new)   → ***REMOVED***252E48   /* menus, popovers, tooltips */
```
Cascades through every component for free. **Effort: S (~1h incl. QA across screens).**

***REMOVED******REMOVED******REMOVED*** 1.6 Sort `<select>` in Sources → styled buttons — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 1.7 Shadow consistency — `shadow-md` / `shadow-2xl` / `shadow-lg` → token — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 1.8 `tabular-nums` missing on numerics — **MEDIUM (polish)**
...
***REMOVED******REMOVED******REMOVED*** 1.9 Replace `confirm()` browser dialog — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 1.10 "Tip — select two or more records to merge" threshold — **LOW**
...
***REMOVED******REMOVED******REMOVED*** 1.11 Sources standing callout — redundant left border — **LOW**
...
***REMOVED******REMOVED******REMOVED*** 1.12 Commit success flash animation — **MEDIUM**
...
***REMOVED******REMOVED*** Tier 2 — Workflow upgrades (multi-day, real product moves)

***REMOVED******REMOVED******REMOVED*** 2.1 Deep-link to a specific value in the workbench — **HIGH (collaborative tool primitive)**
...
***REMOVED******REMOVED******REMOVED*** 2.2 Bulk operations are not actually bulk — **HIGH**
...
***REMOVED******REMOVED******REMOVED*** 2.3 `initStore()` parallelize the 5-call waterfall — **HIGH (perceived perf)**
...
***REMOVED******REMOVED******REMOVED*** 2.4 `refreshDims()` N+1 — add `/api/dimensions?full=true` — **HIGH**
...
***REMOVED******REMOVED******REMOVED*** 2.5 cmd-K command palette / quick-switcher — **HIGH (paid-product table stakes)**
...
***REMOVED******REMOVED******REMOVED*** 2.6 Per-dimension undo stack (no clear-on-switch) — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 2.7 `M` shortcut in cross-dim inbox — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 2.8 All-dim view: expandable review + dashboard deep-link — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 2.9 Group review panel by canonical target + per-row discard — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 2.10 Real first-run experience — **MEDIUM**
...
***REMOVED******REMOVED******REMOVED*** 2.11 Retire canonical: confirm + optimistic undo + `Promise.all` — **SMALL**
...
***REMOVED******REMOVED******REMOVED*** 2.12 Per-slice store subscriptions — **MEDIUM (debt)**
...
***REMOVED******REMOVED******REMOVED*** 2.13 `MasterTables` columns memo depends on `open` (provenance drawer) — **SMALL**
...
***REMOVED******REMOVED*** Tier 3 — Design system maturity (blocked on §6 brand direction for *style*, not structure)
...
***REMOVED******REMOVED******REMOVED*** 3.1 Typography scale gets a 14px step
...
***REMOVED******REMOVED******REMOVED*** 3.2 Three-tier shadow scale
...
***REMOVED******REMOVED******REMOVED*** 3.3 Motion tokens
```
...
***REMOVED******REMOVED******REMOVED*** 3.4 Semantic state tokens
Today everything in the funnel is the same green; "committed/published" should be distinct from "mapped draft":
- `--committed` / `--committed-soft` (cooler teal-green, e.g. `***REMOVED***0E9F8A`)
- `--staged` / `--staged-soft` (distinct from `--warn` which doubles as "needs attention"; e.g. `***REMOVED***C47C18`)

***REMOVED******REMOVED******REMOVED*** 3.5 Line-height tokens
...
***REMOVED******REMOVED******REMOVED*** 3.8 Page header system
...
***REMOVED******REMOVED******REMOVED*** 3.9 Sidebar nav touch target
...
***REMOVED******REMOVED******REMOVED*** 3.10 Schema header opacity → solid
...
***REMOVED******REMOVED*** Tier 4 — Microinteractions and loading states
...
***REMOVED******REMOVED*** §5 — What's already great (don't touch)
...
***REMOVED******REMOVED*** §6 — Decision required: brand direction

Tier 3 *values* (and a slice of Tier 4) depend on which direction you pick. Tiers 0–2 don't.

***REMOVED******REMOVED******REMOVED*** A. "Warm Editorial — Data Newspaper"
Airtable × NYT Data desk. `DM Serif Display` for headings, warmer `***REMOVED***0A0D17` background, `***REMOVED***E04F25` rust accent replacing `***REMOVED***D6336C`, `border-2px` blockprint card edges, newspaper-column Dashboard.
...
***REMOVED******REMOVED******REMOVED*** B. "Linear-Precise — Engineer's Console" *(closest to current trajectory)*
...
***REMOVED******REMOVED******REMOVED*** C. "Data-Dense Terminal-Elevated"
Hex / Warp / Ghostty reference. Berkeley Mono (or JetBrains Mono) for ALL UI text, not just data. Warmer `***REMOVED***0C0E16` OLED-leaning surface. Ledger-book horizontal rules between sections. Sidebar full-mono. **Persistent right-edge status rail** (32px) showing live scan status, commit queue depth, presence dots.
...
***REMOVED******REMOVED*** Suggested execution order
