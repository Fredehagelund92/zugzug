...
describe("mergeRecord soft-deletes the loser's record_version", () => {
  it("retired_at + retired_into are populated on loser rows; survivor stays live", async () => {
    ...
    await addRecordOne(dimId, "United States", "us", USER_ID, T);
    await addRecordOne(dimId, "USA Alias", "usa", USER_ID, T);
    ...
    // Merge: "usa" → "us".
    const merged = await mergeRecord(dimId, "us", ["usa"], USER_ID, expected, T);
...
describe("retireRecord soft-deletes the record_version row", () => {
  it("retired_at is set, retired_into stays null (no merge target)", async () => {
    ...
    await addRecordOne(dimId, "X One", "x1", USER_ID, T);
    ...
    const result = await retireRecord(dimId, "x1", USER_ID, before!.version, T);
...
describe("retired rows do not appear in record listings", () => {
  it("after merge, version-lookup SELECT returns survivor only — no ghost row", async () => {
    ...
    await addRecordOne(dimId, "Alpha", "a", USER_ID, T);
    await addRecordOne(dimId, "Bravo", "b", USER_ID, T);
    ...
    // Merge: "b" → "a".
    const merged = await mergeRecord(dimId, "a", ["b"], USER_ID, expected, T);
    expect(merged).toBe(1);
    ...
    let threw = false;
    try {
      await renameRecord(dimId, "b", "Resurrected", USER_ID, tombstoneVersion, T);
    } catch {
      threw = true;
    }
...
describe("re-adding a previously-retired key un-tombstones the record_version", () => {
  it("after retire(X) then addRecordOne(X), the row is visible with version > 1", async () => {
    ...
    await addRecordOne(dimId, "Phoenix", "phoenix", USER_ID, T);
    ...
    const retired = await retireRecord(dimId, "phoenix", USER_ID, v1, T);
...
describe("retireRecord fires record.deleted outbound event inside the tx", () => {
  ...
  async function cleanup() {
    ...
    for (const id of [DEL_DIM_ID, REF_DIM_ID]) {
      await pgRun(`DELETE FROM "zugzug_app"."record_version" WHERE dim_id = $1`, [id]).catch(
        () => {},
      );
  ...
  it("writes outbound_event with dim_slug/key/label/deleted_by when retire succeeds", async () => {
    ...
    await addRecordOne(dimId, "Beta", undefined, USER_ID, T);
    ...
    const result = await retireRecord(dimId, "beta", USER_ID, v!.version, T);
    expect(result.ok).toBe(true);
  ...
  it("does NOT write an outbound_event when retire is refused due to live variants", async () => {
    ...
    await addRecordOne(dimId, "Gamma", undefined, USER_ID, T);
    ...
    const result = await retireRecord(dimId, "gamma", USER_ID, v!.version, T);
