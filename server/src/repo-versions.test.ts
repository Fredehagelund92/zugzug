...
describe("commit writes a snapshot", () => {
  it("commit writes a snapshot with the same version as the outbound event", async () => {
    const refTableId = await addRefTable("SnapDim", [], { keyKind: "slug" }, U, T);
  ...
  it("version number matches outbound_event count", async () => {
    const refTableId = await addRefTable("VersionCountDim", [], { keyKind: "slug" }, U, T);
    await addRecordOne(refTableId, "Beta", undefined, U, T);
    await saveDraft(refTableId, "beta raw", "mapped", "Beta", "beta", U, T);
    await commit(refTableId, U, T);

    await addRecordOne(refTableId, "Gamma", undefined, U, T);
    await saveDraft(refTableId, "gamma raw", "mapped", "Gamma", "gamma", U, T);
    await commit(refTableId, U, T);
