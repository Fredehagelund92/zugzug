...
beforeEach(async () => {
  ...
  await pgRun(
    `INSERT INTO zugzug_app.tenant (id, slug, label, created_at)
       VALUES ($1, $1, 'test refTable clusters', now())
     ON CONFLICT (id) DO NOTHING`,
    [TENANT],
  );
});

test("getSourceScanValuesAll returns every value worst-impact first when under the cap", async () => {
  await seed(
  ...
  const { rows, truncated } = await getSourceScanValuesAll(TENANT, REF_TABLE, { filter: "new" });
...
test("getSourceScanValuesAll truncates at the cap and flags it", async () => {
  await seed(
    Array.from({ length: 30 }, (_, i) => ({
      raw: `v${String(i).padStart(2, "0")}`,
      rows: 1000 - i,
  ...
  const { rows, truncated } = await getSourceScanValuesAll(TENANT, REF_TABLE, {
    filter: "new",
    cap: 10,
  });
...
test("getRefTableClusters groups look-alikes and reports coverage from whole-refTable scalars", async () => {
  await seed([
    { raw: "USA", rows: 1000 },
    { raw: "U.S.A.", rows: 500 },
    { raw: "GB", rows: 300 },
  ...
  const feed = await getRefTableClusters(TENANT, REF_TABLE, { filter: "new" });
  ...
  // Coverage is whole-refTable: resolved = GB's 300; at risk = 1500; pct = 17.
...
test("TenantRepo.getRefTableClusters scopes to its tenant and returns the feed", async () => {
  await seed([
    { raw: "USA", rows: 1000 },
    { raw: "U.S.A.", rows: 500 },
  ]);
  const repo = new TenantRepo(TENANT, "admin");
  const feed = await repo.getRefTableClusters(REF_TABLE, { filter: "new" });
  expect(feed.clusters).toHaveLength(1);
  expect(feed.clusters[0].key).toBe("usa");
  expect(feed.coverage.pct).toBe(0); // nothing mapped → 0% of 1500 rows resolved
});

test("empty refTable reports 100% coverage (nothing at risk) with no clusters", async () => {
  await seed([]); // refTable exists, zero scan values
  const feed = await getRefTableClusters(TENANT, REF_TABLE, { filter: "new" });
