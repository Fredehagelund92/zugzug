...
beforeAll(async () => {
  // Provision a fake refTable + dim_X table the backfill DO-block can find.
  ...
  await pgRun(`DELETE FROM "zugzug_app"."record_version" WHERE reference_table_id = 'd_test_e2'`);
  await pgRun(
...
afterAll(async () => {
  // Clean up so the API and the next test run don't see a phantom refTable. The
  // dim_X/map_X tables can stay — they're orphaned but invisible without the
  // registry entry.
  await pgRun(`DELETE FROM "zugzug_app"."record_version" WHERE reference_table_id = 'd_test_e2'`);
  await pgRun(`DELETE FROM "zugzug_app"."reference_table" WHERE id = 'd_test_e2'`);
});

test("record_version table exists and is empty for the test refTable before backfill", async () => {
  // (The migration already ran in db:migrate above; for this test we re-run
  //  just the backfill block to simulate "what if a new refTable was added later".)
  await pgRun(`DELETE FROM "zugzug_app"."record_version" WHERE reference_table_id = 'd_test_e2'`);
  const empty = await pgGet<{ n: number }>(
    `SELECT count(*)::int AS n FROM "zugzug_app"."record_version" WHERE reference_table_id = 'd_test_e2'`,
  );
  expect(empty?.n).toBe(0);
});

test("backfill seeds version=1 for every existing refTable row", async () => {
  // Apply the same DO-block the migration uses (idempotent).
  await pgRun(`
    DO $$
    DECLARE d record; sql_stmt text;
    BEGIN
      FOR d IN SELECT id, dim_table, key_col, tenant_id FROM "zugzug_app"."reference_table" WHERE id = 'd_test_e2' LOOP
        sql_stmt := format(
  ...
  const rows = await pgAll<{ key: string; version: number }>(
    `SELECT key, version FROM "zugzug_app"."record_version"
     WHERE reference_table_id = 'd_test_e2' ORDER BY key`,
  );
...
test("backfill is idempotent — re-running does not duplicate or bump version", async () => {
  ...
  await pgRun(
    `UPDATE "zugzug_app"."record_version" SET version = 7
       WHERE reference_table_id = 'd_test_e2' AND key = 'dk'`,
  );
