...
/* Master tables (pillar 2) — the golden-record workbench, live against Postgres
   dim_/map_. Derive canonical from a source column, MERGE near-duplicates into one
   survivor (variants re-point), rename, govern-retire, and enrich with attribute
   COLUMNS (region, currency, …) editable inline. Expand a record for its raw
   variants (the lineage receipt). Every mutation is persisted + audited. */
...
export function MasterTables() {
  ...
  // grid widens with each enrichment column: checkbox · label · key · …fields · variants · actions
  ...
  const toggleOpen = async (key: string) => {
    ...
    if (!variantsCache[ck(key)]) {
      ...
      const vs = await fetchVariants(dimId, key);
      setVariantsCache((c) => ({ ...c, [ck(key)]: vs }));
    }
  };

  const add = async () => { const label = draft.trim(); if (!label || busy) return; setBusy(true); await addCanonical(dimId, label); setBusy(false); setDraft(""); };
  const rename = async (key: string, next: string) => { setEditing(null); const label = next.trim(); if (!label) return; setBusy(true); await renameCanonical(dimId, key, label); setBusy(false); };
  const merge = async (survivorLabel: string) => {
    ...
    setBusy(true); const n = await mergeCanonical(dimId, survivor, losers); setBusy(false);
    setSel([]); flash(`Merged ${n} record${n === 1 ? "" : "s"} into ${survivorLabel} — variants re-pointed.`);
  };
  const retire = async (key: string, label: string) => {
    setBusy(true); const r = await retireCanonical(dimId, key); setBusy(false);
    if (!r.ok) flash(`Can’t retire “${label}” — ${r.variants} variant${r.variants === 1 ? "" : "s"} still map here. Merge or remap them first.`);
  };
  const derive = async (opt: string) => {
    const s = wired.find((w) => `${w.table}.${w.column}` === opt);
    if (!s || busy) return;
    setBusy(true); const n = await deriveCanonical(dimId, s.table, s.column); setBusy(false);
    flash(n > 0 ? `Seeded ${n} canonical value${n === 1 ? "" : "s"} from ${s.table}.${s.column}.` : `${s.table}.${s.column} has no distinct values to derive (no rows).`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="zz-rise relative z-40 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="font-mono text-[10px] font-medium uppercase tracking-[0.22em] text-ink-3">Canonical</div>
          <h1 className="mt-1.5 font-display text-[clamp(28px,4vw,44px)] font-extrabold leading-none tracking-[-0.035em] text-ink">Master tables</h1>
          <p className="mt-3 max-w-[60ch] text-ink-2">The golden records every source resolves to. Derive them from a source, merge duplicates, add attribute columns, and expand any record for its raw variants.</p>
        </div>
      ...
      <div className="zz-rise relative z-30" style={{ animationDelay: "60ms" }}>
        <DimensionPicker dims={dims} activeId={dimId}
      ...
      <div className="zz-rise flex flex-wrap items-center gap-x-6 gap-y-2 rounded-lg border border-line bg-surface px-5 py-4 font-mono text-[11px]" style={{ animationDelay: "100ms" }}>
        ...
        <div className="ml-auto flex items-center gap-4">
          <span className="text-ink-3">{totalVariants.toLocaleString()} variants resolve here</span>
          <AddColumn onAdd={(label, type) => addField(dimId, label, type)} />
        </div>
      </div>
      ...
      <div className="zz-rise overflow-x-auto rounded-lg border border-line bg-surface" style={{ animationDelay: "150ms" }}>
        ...
        <div className="flex flex-wrap items-center gap-3 border-b border-line px-5 py-2.5">
          {sel.length === 0 ? (
            <div className="grid w-full items-center gap-3 font-mono text-[10px] uppercase tracking-wider text-ink-3" style={gridStyle}>
              <span /><span>Canonical record</span><span>{dim.keyCol}</span>
              {fields.map((f) => <span key={f.field} className="truncate" title={f.label}>{f.label}</span>)}
              <span className="text-right">Variants</span><span />
            </div>
          ) : (
            <>
          ...
          return (
            <Fragment key={c.key}>
              <div className={cx("grid items-center gap-3 border-b border-line px-5 py-3 transition-colors", checked ? "bg-accent-wash" : "hover:bg-hover", isOpen && "border-b-0")} style={gridStyle}>
                ...
                {fields.map((f) => (
                  <FieldCell key={f.field} value={c.fields?.[f.field] ?? null} type={f.type} onCommit={(v) => setFieldValue(dimId, c.key, f.field, v)} />
                ))}
                ...
                <div className="flex justify-end gap-1.5">
                  ...
                  <button type="button" aria-label={locked ? "Has variants — can't retire" : "Retire"} title={locked ? `${variants} variants still map here` : "Retire record"}
              ...
              {isOpen && (
                <div className="border-b border-line bg-surface-2/40 px-5 py-3 pl-[44px]">
                  <div className="font-mono text-[10px] uppercase tracking-wider text-ink-3">raw variants mapped here (the receipt)</div>
                  {cached === "loading" ? (
                  ...
                  ) : <div className="mt-2 font-mono text-[11px] text-ink-3">no source values map here yet — reconcile on Value mapping</div>}
                </div>
              )}
            </Fragment>
          );
        ...
        {list.length === 0 && (
          <div className="px-5 py-12 text-center font-mono text-[12px] text-ink-3">no canonical records yet — derive from a source above, or add one below</div>
        )}
