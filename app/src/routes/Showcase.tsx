...
function Section({ title, hint, delay, children }: { title: string; hint?: string; delay: number; children: React.ReactNode }) {
  return (
  ...
  return (
    <div className="min-h-screen">
      <header className="sticky top-0 z-50 flex items-center justify-between border-b border-line bg-[var(--ak-glass)] px-6 py-3.5 backdrop-blur-md">
        <Link to="/app" className="flex items-center gap-2.5 font-display text-base font-extrabold tracking-tight text-ink">
        ...
        <main className="mx-auto max-w-[var(--maxw)] space-y-16 px-8 py-16">
          <div className="zz-rise space-y-5">
            <div className="font-mono text-[11px] uppercase tracking-[0.2em] text-ink-3">
              <span className="text-accent">[ </span>design system · live<span className="text-accent"> ]</span>
            </div>
            ...
            <p className="max-w-[52ch] text-lg text-ink-2">
              The Zug Zug component layer, assembled in Tailwind from the brand
              tokens. Switch the accent or theme above — every component recolors
              from a single token, never a hardcoded value.
            </p>
          ...
          <Section title="Color tokens" hint="accent fixed to brand · BrandApp.setAccent() in console" delay={150}>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-5">
              {swatches.map((s) => (
                <div key={s.label}>
                  <div className={cx("h-16 rounded-md border", s.border ? "border-line-2" : "border-line", s.cls)} />
          ...
          <Section title="Typography" hint="Bricolage · Hanken · JetBrains" delay={210}>
            <div className="space-y-5 rounded-lg border border-line bg-surface p-8">
              <p className="font-display text-[64px] font-extrabold leading-none tracking-[-0.04em] text-ink">Aa</p>
              ...
              <p className="max-w-[60ch] text-base text-ink-2">
                Body copy is set in Hanken Grotesk at a comfortable measure. Headlines
                use Bricolage Grotesque; anything tabular uses JetBrains Mono so columns
                line up and data stays honest.
              </p>
              <p className="font-mono text-sm text-ink-3">dim_customer · 1.24M rows · 98.7% mapped · 28 cols</p>
            </div>
          </Section>
            ...
            <div className="space-y-4 rounded-lg border border-line bg-surface p-8">
              {(["primary", "secondary", "ghost", "danger"] as const).map((v) => (
                <div key={v} className="flex flex-wrap items-center gap-3">
                  <span className="w-20 font-mono text-[11px] uppercase tracking-wider text-ink-3">{v}</span>
                  <Button variant={v} size="lg">Action</Button>
          ...
          <Section title="Badges" hint="status" delay={330}>
            <div className="flex flex-wrap gap-3 rounded-lg border border-line bg-surface p-8">
              ...
              <Badge tone="ok" dot>Mapped</Badge>
              <Badge tone="warn" dot>Review</Badge>
              <Badge tone="danger" dot>Unmapped</Badge>
              <Badge>neutral</Badge>
            </div>
          </Section>
        </main>
