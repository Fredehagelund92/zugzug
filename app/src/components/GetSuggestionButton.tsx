...
/** Calls POST /api/dimensions/:id/suggest for an unmapped value, shows a
...
export function GetSuggestionButton({ dimensionId, rawValue }: Props) {
  const [loading, setLoading] = useState(false);

  const handleClick = async () => {
    setLoading(true);
    try {
      const result = await generateSuggestion(dimensionId, rawValue);
