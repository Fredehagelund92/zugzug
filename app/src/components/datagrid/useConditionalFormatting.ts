...
function evaluateTrigger(rule: ConditionalRule, raw: unknown): boolean {
  ...
  switch (rule.trigger.kind) {
    case "equals":
      return s === rule.trigger.value;
    case "not_equals":
      return s !== rule.trigger.value;
    case "contains":
      return s.includes(rule.trigger.value);
    case "starts_with":
      return s.startsWith(rule.trigger.value);
    case "ends_with":
      return s.endsWith(rule.trigger.value);
    case "is_in":
      return rule.trigger.values.includes(s);
    case "gt":
      return typeof raw === "number" && raw > rule.trigger.value;
    case "lt":
      return typeof raw === "number" && raw < rule.trigger.value;
    case "between":
      return typeof raw === "number" && raw >= rule.trigger.min && raw <= rule.trigger.max;
  }
}
