...
***REMOVED******REMOVED*** The trust chain (why it can't drift)
...
4. The gate proves it:
   ```
   python3 ../../brand/../?  ***REMOVED*** from repo root:
...
```
bun install
bun run dev        ***REMOVED*** http://localhost:5173
bun run build      ***REMOVED*** production bundle → dist/
bun run typecheck
```
