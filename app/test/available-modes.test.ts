...
describe("availableModes", () => {
  ...
  test("sourced + mapped table → records, match", () => {
    expect(availableModes(refTable("a"), [src("a")])).toEqual(["records", "match"]);
  });
  ...
  test("multiple wired sources for the same refTable still yield one match entry", () => {
    expect(availableModes(refTable("a"), [src("a"), src("a")])).toEqual(["records", "match"]);
  });
});
